...
def make_raw(size: int = 64, *, magic: bytes = TELINK_RAW_MAGIC) -> bytes:
    if size < 0x1C:
        raise ValueError
...
class TelinkFinalizeTests(unittest.TestCase):
    def test_finalize_raw_tc32_link_binary_writes_magic(self) -> None:
        raw = make_raw(64)
        self.assertEqual(raw[6:8], TELINK_RAW_MAGIC)
        validate_link_binary(raw)
        final = finalize_link_binary(raw)
        self.assertEqual(final[6:8], TELINK_8258_MAGIC)
    ...
    def test_accept_prefilled_magic_link_binary(self) -> None:
        raw = make_raw(magic=TELINK_8258_MAGIC)
        validate_link_binary(raw)
        final = finalize_link_binary(raw)
        self.assertEqual(final[6:8], TELINK_8258_MAGIC)
    ...
    def test_reject_unexpected_raw_magic(self) -> None:
        with self.assertRaises(TelinkImageError):
            validate_link_binary(make_raw(magic=b"\xaa\xbb"))
    ...
    def test_parse_final_preamble(self) -> None:
        p = parse_preamble(finalize_link_binary(make_raw()))
