...
DEFAULT_FILE_VERSION = 0x7F020001
...
def parse_preamble(data: bytes | bytearray, *, require_final_magic: bool = True) -> TelinkPreamble:
    ...
    if require_final_magic:
        if magic != TELINK_8258_MAGIC:
            raise TelinkImageError(f"unexpected MCU magic {magic.hex()}; expected {TELINK_8258_MAGIC.hex()}")
    elif magic not in (TELINK_RAW_MAGIC, TELINK_8258_MAGIC):
        raise TelinkImageError(f"unexpected raw linker magic {magic.hex()}; expected 0000 or {TELINK_8258_MAGIC.hex()}")
...
def validate_identity(preamble: TelinkPreamble, *, manufacturer_code: int = DEFAULT_MANUFACTURER,
                      image_type: int = DEFAULT_IMAGE_TYPE, file_version: int | None = DEFAULT_FILE_VERSION) -> None:
    if preamble.manufacturer_code != manufacturer_code:
        raise TelinkImageError(f"manufacturer mismatch: 0x{preamble.manufacturer_code:04x} != 0x{manufacturer_code:04x}")
    if preamble.image_type != image_type:
        raise TelinkImageError(f"image type mismatch: 0x{preamble.image_type:04x} != 0x{image_type:04x}")
    if file_version is not None and preamble.file_version != file_version:
        raise TelinkImageError(f"file version mismatch: 0x{preamble.file_version:08x} != 0x{file_version:08x}")
...
def is_valid_finalized_image(data: bytes | bytearray) -> bool:
    ...
    if len(data) < TELINK_MIN_HEADER + 4 or preamble.declared_size != len(data):
        return False
    return telink_xcrc32(data[:-4]) == _u32le(data, len(data) - 4)

def validate_link_binary(data: bytes, *, manufacturer_code: int = DEFAULT_MANUFACTURER,
                         image_type: int = DEFAULT_IMAGE_TYPE, file_version: int | None = DEFAULT_FILE_VERSION,
                         max_final_size: int = 0x34000) -> TelinkPreamble:
    preamble = parse_preamble(data, require_final_magic=False)
    validate_identity(preamble, manufacturer_code=manufacturer_code, image_type=image_type, file_version=file_version)
    if preamble.declared_size != len(data):
        raise TelinkImageError(f"raw linker size mismatch: header declares {preamble.declared_size}, file has {len(data)}")
    ...
    if padded + 4 >= max_final_size:
        raise TelinkImageError(f"final image would reach/exceed 0x{max_final_size:x} app limit")
    return preamble

def finalize_link_binary(data: bytes, *, manufacturer_code: int = DEFAULT_MANUFACTURER,
                         image_type: int = DEFAULT_IMAGE_TYPE, file_version: int | None = DEFAULT_FILE_VERSION,
                         max_final_size: int = 0x34000) -> bytes:
    validate_link_binary(data, manufacturer_code=manufacturer_code, image_type=image_type,
                         file_version=file_version, max_final_size=max_final_size)
    body = bytearray(data)
    padding = (-len(body)) % 16
    if padding:
        body.extend(b"\xff" * padding)
    body[6:8] = TELINK_8258_MAGIC
    struct.pack_into("<I", body, TELINK_DECLARED_SIZE_OFFSET, len(body) + 4)
    result = bytes(body) + struct.pack("<I", telink_xcrc32(body))
    validate_finalized_image(result, manufacturer_code=manufacturer_code, image_type=image_type,
                             file_version=file_version, max_final_size=max_final_size)
    return result

def validate_finalized_image(data: bytes, *, manufacturer_code: int = DEFAULT_MANUFACTURER,
                             image_type: int = DEFAULT_IMAGE_TYPE, file_version: int | None = DEFAULT_FILE_VERSION,
                             max_final_size: int = 0x34000) -> TelinkPreamble:
    preamble = parse_preamble(data, require_final_magic=True)
    validate_identity(preamble, manufacturer_code=manufacturer_code, image_type=image_type, file_version=file_version)
    if preamble.declared_size != len(data):
        raise TelinkImageError(f"final size mismatch: header declares {preamble.declared_size}, file has {len(data)}")
    ...
    if actual != expected:
        raise TelinkImageError(f"Telink xcrc32 mismatch: computed 0x{actual:08x}, tail 0x{expected:08x}")
    return preamble
...
def main(argv: list[str] | None = None) -> int:
    ...
    kwargs = dict(manufacturer_code=args.manufacturer, image_type=args.image_type,
                  file_version=args.file_version, max_final_size=args.max_final_size)
    try:
        if args.mode == "check-link":
            p = validate_link_binary(data, **kwargs)
            print(f"TELINK_LINK_IMAGE=PASS bytes={len(data)} fileVersion=0x{p.file_version:08x}")
        elif args.mode == "finalize":
            if args.output is None:
                parser.error("finalize requires OUTPUT")
            result = finalize_link_binary(data, **kwargs)
            args.output.write_bytes(result)
            print(f"TELINK_FINALIZE=PASS bytes={len(result)} xcrc32=0x{_u32le(result, len(result)-4):08x}")
        else:
            p = validate_finalized_image(data, **kwargs)
            print(f"TELINK_FINAL_IMAGE=PASS bytes={len(data)} fileVersion=0x{p.file_version:08x}")
