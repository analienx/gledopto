...
#define GLSD301P_ENDPOINT                    0x0Bu
...
static ev_timer_event_t *g_io_timer;
...
/* Basic attributes: custom implementation identity, stock-compatible hardware lineage. */
                                   ...
                                   u8 frame[GLSD301P_CONTROL_FRAME_SIZE])
{
    ...
    if (!glsd_uart_send(frame)) {
        ...
        glsd301p_runtime_core_latch_fault(&g_runtime, off);
...
static s32 glsd_level_timer_cb(void *arg)
{
    ...
    if (g_level_transition.mode == GLSD_LEVEL_IDLE || !glsd301p_runtime_core_is_ready(&g_runtime)) {
        g_level_timer = NULL;
        g_remaining_time = 0u;
        return -1;
    }
    ...
    if (g_level_transition.mode == GLSD_LEVEL_TARGET) {
        ...
        if (g_remaining_time <= 1u) {
            ...
            u16 step = (diff + g_remaining_time - 1u) / g_remaining_time;
            if (step == 0u) {
                step = 1u;
            }
            if (g_current_level < g_level_transition.target) {
                next = glsd_clamp_level((u16)g_current_level + step);
            } else {
                next = glsd_clamp_level((u16)g_current_level - step);
            }
            g_remaining_time--;
    ...
    g_level_transition.rate_accum_tenths =
        (u8)(g_level_transition.rate_accum_tenths + g_level_transition.rate);
    {
...
static void glsd_start_target_transition(u8 target, u16 transition_time, u8 with_onoff)
{
    ...
    if (transition_time == 0u || transition_time == 0xFFFFu || target == g_current_level) {
    ...
    g_level_timer = TL_ZB_TIMER_SCHEDULE(glsd_level_timer_cb, NULL, GLSD301P_LEVEL_TICK_MS);
}
...
static status_t glsd_onoff_cb(zclIncomingAddrInfo_t *addr, u8 cmd_id, void *payload)
{
    ...
    if (!addr || addr->dstEp != GLSD301P_ENDPOINT) {
...
static status_t glsd_level_cb(zclIncomingAddrInfo_t *addr, u8 cmd_id, void *payload)
{
    if (!addr || addr->dstEp != GLSD301P_ENDPOINT || !payload) {
        return ZCL_STA_INVALID_FIELD;
    }
    ...
    case ZCL_CMD_LEVEL_STEP_WITH_ON_OFF: {
        ...
        if (cmd->stepMode == LEVEL_STEP_UP) {
            ...
            target = (target > cmd->stepSize) ? (target - cmd->stepSize) : g_min_level;
        }
        glsd_start_target_transition((u8)glsd_clamp_level(target), cmd->transitionTime,
...
static s32 glsd_io_timer_cb(void *arg)
{
    ...
    result = glsd301p_runtime_core_poll_push(&g_runtime, drv_gpio_read(GPIO_PC2), frame);
    if (result != GLSD301P_RUNTIME_NO_FRAME) {
        (void)glsd_emit_runtime_result(result, frame);
    }

    result = glsd301p_runtime_core_poll_pb4(&g_runtime, drv_gpio_read(GPIO_PB4), frame);
    if (result != GLSD301P_RUNTIME_NO_FRAME) {
        (void)glsd_emit_runtime_result(result, frame);
    }
...
static void glsd_init_power_stage_io(void)
{
    u8 frame[GLSD301P_CONTROL_FRAME_SIZE];
    ...
    g_uart_online = (drv_uart_init(GLSD301P_TARGET_UART_BAUD,
                                   g_uart_rx_buf,
                                   sizeof(g_uart_rx_buf),
                                   NULL) == 0u);

    if (g_uart_online) {
        if (!glsd_uart_send(frame)) {
            /* frame is populated immediately below; keep compiler flow simple. */
        }
        if (glsd301p_runtime_core_boot_off(&g_runtime, frame) != GLSD301P_RUNTIME_FRAME_READY ||
            !glsd_uart_send(frame)) {
            glsd301p_runtime_core_latch_fault(&g_runtime, frame);
        }
    } else {
        glsd301p_runtime_core_latch_fault(&g_runtime, frame);
    }
...
void user_init(bool isRetention)
{
    ...
    /* Electrical safety path comes first. */
    glsd_init_power_stage_io();
    ...
    zcl_init(NULL);
    af_endpointRegister(GLSD301P_ENDPOINT, (af_simple_descriptor_t *)&g_simple_desc,
                        zcl_rx_handler, NULL);
    if (zcl_register(GLSD301P_ENDPOINT,
                     sizeof(g_cluster_list) / sizeof(g_cluster_list[0]),
                     (zcl_specClusterInfo_t *)g_cluster_list) == ZCL_STA_SUCCESS) {
        g_zcl_online = 1u;
    }
    ...
    /* Prototype target intentionally restores to a known OFF state. Persistent
     * power-on behavior remains a production gate, not an inferred behavior. */
    if (g_uart_online &&
        glsd301p_runtime_core_restore_state(&g_runtime, false,
                                            GLSD301P_MAX_LEVEL,
                                            GLSD301P_MIN_LEVEL,
                                            false, frame) == GLSD301P_RUNTIME_FRAME_READY) {
        (void)glsd_emit_runtime_result(GLSD301P_RUNTIME_FRAME_READY, frame);
    }

    g_io_timer = TL_ZB_TIMER_SCHEDULE(glsd_io_timer_cb, NULL, GLSD301P_IO_POLL_MS);
