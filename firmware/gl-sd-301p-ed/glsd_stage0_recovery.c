 ...
 * are therefore outside executable Stage-0 bytes and below MAC/factory/NV2. */
#define GLSD_STAGE0_BACKUP_SECTOR    0x70000u
#define GLSD_STAGE0_JOURNAL_SECTOR   0x71000u

#define GLSD_STAGE0_JOURNAL_MAGIC    0x30425253u /* "SRB0" little-endian */
#define GLSD_STAGE0_JOURNAL_VERSION  1u
...
static bool glsd_flag_family_valid(u32 word)
{
    return (word & 0xffffff00u) == (GLSD_TELINK_START_WORD & 0xffffff00u);
}
                                      ...
                                      u32 *out_crc)
{
    ...
    flash_read(GLSD_BANK_A_BASE + image_size - sizeof(stored_crc),
    ...
    while (remaining) {
        ...
        flash_read(GLSD_BANK_A_BASE + offset, length, g_chunk);
        if (offset == 0u) {
            /* Telink's OTA CRC check normalizes the one-byte boot marker. */
...
static bool glsd_load_valid_journal(glsd_stage0_journal_t *journal)
{
    ...
    flash_read(GLSD_STAGE0_JOURNAL_SECTOR, sizeof(*journal), (u8 *)journal);
    if (journal->magic != GLSD_STAGE0_JOURNAL_MAGIC ||
        ...
        !glsd_flag_family_valid(journal->source_flag_word) ||
        journal->record_crc != glsd_journal_record_crc(journal)) {
        return false;
    }

    for (offset = 0; offset < GLSD_FLASH_SECTOR_SIZE; offset += GLSD_CHUNK_SIZE) {
        flash_read(GLSD_STAGE0_BACKUP_SECTOR + offset, GLSD_CHUNK_SIZE, g_chunk);
                                           ...
                                           u32 source_flag)
{
    ...
    flash_read(GLSD_BANK_A_BASE, GLSD_FLASH_SECTOR_SIZE, g_sector);
    for (offset = 0; offset < GLSD_FLASH_SECTOR_SIZE; offset += GLSD_CHUNK_SIZE) {
        backup_crc = xcrc32(g_sector + offset, GLSD_CHUNK_SIZE, backup_crc);
    }

    if (!glsd_program_sector(GLSD_STAGE0_BACKUP_SECTOR, g_sector)) {
        return false;
    }
    ...
    flash_unlock();
    flash_erase(GLSD_STAGE0_JOURNAL_SECTOR);
    if (!flash_writeWithCheck(GLSD_STAGE0_JOURNAL_SECTOR,
                              sizeof(journal), (u8 *)&journal)) {
        flash_lock();
        return false;
    }
    flash_lock();
    ...
    flash_read(GLSD_STAGE0_JOURNAL_SECTOR, sizeof(journal), g_chunk);
...
static bool glsd_restore_stock_sector(const glsd_stage0_journal_t *journal)
{
    ...
    flash_read(GLSD_STAGE0_BACKUP_SECTOR, GLSD_FLASH_SECTOR_SIZE, g_sector);
    for (offset = 0; offset < GLSD_FLASH_SECTOR_SIZE; offset += GLSD_CHUNK_SIZE) {
...
static bool glsd_invalidate_stage0_bank(void)
{
    ...
    if (glsd_read_u32(GLSD_BANK_B_BASE + GLSD_BOOT_FLAG_OFFSET) !=
        GLSD_TELINK_START_WORD) {
        return false;
    }

    flash_unlock();
    if (!flash_writeWithCheck(GLSD_BANK_B_BASE + GLSD_BOOT_FLAG_OFFSET, 1u, &zero)) {
        flash_lock();
        return false;
    }
    flash_lock();

    word = glsd_read_u32(GLSD_BANK_B_BASE + GLSD_BOOT_FLAG_OFFSET);
    return word == GLSD_TELINK_DISABLED_WORD &&
           glsd_read_u32(GLSD_BANK_A_BASE + GLSD_BOOT_FLAG_OFFSET) ==
               GLSD_TELINK_START_WORD;
}
...
glsd_stage0_recovery_status_t glsd_stage0_recovery_prepare(void)
{
    ...
    /* Stage-0 is intentionally valid in bank B when first entered. */
    if (glsd_read_u32(GLSD_BANK_B_BASE + GLSD_BOOT_FLAG_OFFSET) !=
        GLSD_TELINK_START_WORD) {
        return GLSD_STAGE0_RECOVERY_ERR_BOOT_LAYOUT;
    }
    ...
    if (!journal_valid) {
        source_flag = glsd_read_u32(GLSD_BANK_A_BASE + GLSD_BOOT_FLAG_OFFSET);
    ...
    /* Power may have failed after a previous restore but before Stage-0's own
     * flag was cleared. Avoid another erase if stock A is already fully valid. */
    ...
    /* This is the commit point. It is deliberately last and only changes 1->0.
     * Before it, Stage-0 B remains bootable. After it, verified stock A is the
     * only bootable bank, so any reset/power loss returns to stock. */
